...
import 'package:harvest/ViewItems.dart';
            ...
            return Card(
                ...
                child: InkWell(
                  onTap: () async {
                    ...
                    await prefs.setString('log_id',list[position]['log_id']);

                    print(list[position]['log_id']);

                    http.Response response = await http.post(Uri.parse("http://10.100.15.123/viewlogrecords.php"),
                      body: ({
                        'log_id': list[position]['log_id']
                       })    
                    );

                    if(response.statusCode == 200){

                      List logData = json.decode(response.body); 

                      Navigator.push( context,
                        MaterialPageRoute(
                          builder: (context) =>  ViewItems(logData)
                        ),
                      );
...
//   //  {Key? key, required this.list}) : super(key: key);
